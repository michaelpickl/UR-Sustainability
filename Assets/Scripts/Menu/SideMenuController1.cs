...
/*
    ...
    {
        Transform valueUnitTransform = sliderObject.transform.Find("ValueUnit");
        if (valueUnitTransform != null)
        {
            TextMeshProUGUI valueUnitText = valueUnitTransform.GetComponent<TextMeshProUGUI>();
            if (valueUnitText != null)
            {
                valueUnitText.text = consumers.monthly_values.ToString();  
            }
        }
    }

     void SetSliderValues(string buildingName)
    {

        if (building != null)
        {
            foreach (var consumer in building.consumers)
            {
                if (consumer.type == "Strom")
                {
                    float stromValue = consumer.monthly_values;
                    //stromSlider.value = stromValue.ToString();

                    // Text aktualisieren, der den Slider-Wert anzeigt
                    stromText.text = $"Strom: {stromValue:F2} CO2";
                }
            }
        }
    }
